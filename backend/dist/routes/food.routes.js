...
router.get('/', async (req, res) => {
    try {
        ...
        const foodItems = await db_1.prisma.foodItem.findMany({
            where,
...
router.get('/:id', async (req, res) => {
    try {
        const foodItem = await db_1.prisma.foodItem.findUnique({
...
router.patch('/:id/toggle-availability', async (req, res) => {
    try {
        const existing = await db_1.prisma.foodItem.findUnique({ where: { id: req.params.id } });
        if (!existing) {
            return res.status(404).json({ error: 'Food item not found' });
        }
        const updated = await db_1.prisma.foodItem.update({
            where: { id: req.params.id },
...
router.delete('/:id', async (req, res) => {
    try {
        await db_1.prisma.foodItem.delete({ where: { id: req.params.id } });
