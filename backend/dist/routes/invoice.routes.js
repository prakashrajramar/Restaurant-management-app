...
router.get('/', async (req, res) => {
    try {
        ...
        if (from || to)
            where.createdAt = {};
        if (from)
            where.createdAt.gte = new Date(String(from) + 'T00:00:00');
        if (to)
            where.createdAt.lte = new Date(String(to) + 'T23:59:59.999');
        const invoices = await db_1.prisma.invoice.findMany({ where, include: { customer: true, table: true, items: { include: { foodItem: true } } }, orderBy: { createdAt: 'desc' } });
        res.json(invoices);
    }
    catch (error) {
        console.error(error);
...
router.get('/:id', async (req, res) => {
    try {
        const invoice = await db_1.prisma.invoice.findUnique({ where: { id: req.params.id }, include: { customer: true, table: true, items: { include: { foodItem: true } } } });
        if (!invoice)
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!Array.isArray(items) || items.length === 0)
            return res.status(400).json({ error: 'Invoice must contain at least one food item' });
        const settings = (0, settings_1.getSettings)();
        let finalCustomerId = customerId;
        if (!finalCustomerId && customerPhone) {
            let cust = await db_1.prisma.customer.findUnique({ where: { phone: customerPhone } });
            if (!cust && customerName)
                cust = await db_1.prisma.customer.create({ data: { name: customerName, phone: customerPhone } });
            if (cust)
                finalCustomerId = cust.id;
        }
        if (tableId) {
            const table = await db_1.prisma.restaurantTable.findUnique({ where: { id: tableId } });
            if (!table)
                return res.status(400).json({ error: 'Selected table not found' });
            if (table.status !== 'AVAILABLE' && table.status !== 'OCCUPIED')
                return res.status(400).json({ error: 'Selected table is not available for billing' });
        }
        let subtotal = 0;
        ...
        for (const item of items) {
            ...
            const foodItem = await db_1.prisma.foodItem.findUnique({ where: { id: item.foodItemId } });
            if (!foodItem || !foodItem.isAvailable)
                return res.status(400).json({ error: `Food item ${item.foodItemId} is unavailable` });
        ...
        if (!Number.isFinite(recAmount) || recAmount < grandTotal)
            return res.status(400).json({ error: 'Amount received is less than the bill total' });
        const change = Math.round((recAmount - grandTotal) * 100) / 100;
        const latest = await db_1.prisma.invoice.findFirst({ where: { invoiceNumber: { startsWith: `INV-${new Date().getFullYear()}-` } }, orderBy: { createdAt: 'desc' } });
        ...
        const invoice = await db_1.prisma.$transaction(async (tx) => {
            ...
            for (const item of invoiceItemData)
                await tx.foodItem.update({ where: { id: item.foodItemId }, data: { totalSold: { increment: item.quantity } } });
            ...
            if (finalCustomerId)
                await tx.customer.update({ where: { id: finalCustomerId }, data: { totalVisits: { increment: 1 }, totalSpent: { increment: grandTotal } } });
            return created;
        });
