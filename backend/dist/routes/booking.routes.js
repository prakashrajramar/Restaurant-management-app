...
router.post('/', async (req, res) => {
    try {
        ...
        if (!finalCustomerId && customerPhone && customerName) {
            ...
            if (!cust) {
                cust = await db_1.prisma.customer.create({
        ...
        if (tableId) {
            ...
            if (table.status === 'OCCUPIED' || table.status === 'CLEANING')
                return res.status(400).json({ error: 'Selected table is currently unavailable' });
            if (Number(guests) > table.capacity)
                return res.status(400).json({ error: `Table capacity is ${table.capacity}` });
            const overlap = await db_1.prisma.booking.findFirst({
        ...
        if (tableId) {
            await db_1.prisma.restaurantTable.update({
                where: { id: tableId },
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const booking = await db_1.prisma.booking.findUnique({ where: { id: req.params.id } });
        if (!booking) {
