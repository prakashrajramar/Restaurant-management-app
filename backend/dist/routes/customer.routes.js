...
router.post('/', async (req, res) => {
    try {
        ...
        const existing = await db_1.prisma.customer.findUnique({ where: { phone } });
        if (existing) {
...
router.put('/:id', async (req, res) => {
    try {
        const { name, phone, email, address, notes } = req.body;
        const existing = await db_1.prisma.customer.findFirst({ where: { phone, NOT: { id: req.params.id } } });
        if (existing)
            return res.status(400).json({ error: 'A customer with this phone number already exists' });
...
router.delete('/:id', async (req, res) => {
    try {
        await db_1.prisma.customer.delete({ where: { id: req.params.id } });
