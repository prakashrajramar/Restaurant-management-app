...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-surface-container-highest pb-4 mb-6">
      <div>
        <h2 className="font-display-sm text-2xl sm:text-3xl font-bold text-on-surface">{title}</h2>
        <p className="font-body-lg text-sm sm:text-base text-on-surface-variant mt-1">{subtitle}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-end">
        ...
        <div className="flex items-center border border-outline-variant rounded-lg px-3 py-2 bg-surface-container-lowest focus-within:border-secondary transition-colors w-full sm:w-64">
          ...
          />
          {searchQuery && (
            <button onClick={() => onSearchChange('')} className="text-on-surface-variant hover:text-primary">
        ...
        {/* User avatar */}
        <div className="w-10 h-10 rounded-full bg-primary-container text-on-primary font-bold flex items-center justify-center border border-outline-variant shadow-sm" title="Admin User">
          PR
        </div>
      </div>
    </header>
