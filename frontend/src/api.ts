...
import { FoodItem, Category, Customer, RestaurantTable, Booking, Invoice, DashboardData, RestaurantSettings } from './types';

const API_BASE = '/api';

export const api = {
  // Food Items
  getFoodItems: async (params?: { search?: string; categoryId?: string; isVeg?: boolean; isAvailable?: boolean }) => {
  ...
  toggleFoodAvailability: async (id: string) => {
    const res = await axios.patch<FoodItem>(`${API_BASE}/food/${id}/toggle-availability`);
    return res.data;
  },
  ...
  getCustomers: async (search?: string) => {
    const res = await axios.get<Customer[]>(`${API_BASE}/customers`, { params: { search } });
    return res.data;
  },
  ...
  updateCustomer: async (id: string, data: Partial<Customer>) => {
    const res = await axios.put<Customer>(`${API_BASE}/customers/${id}`, data);
    return res.data;
  },
  ...
  createTable: async (data: Partial<RestaurantTable>) => {
    const res = await axios.post<RestaurantTable>(`${API_BASE}/tables`, data);
    return res.data;
  },
  updateTableStatus: async (id: string, status: string) => {
    const res = await axios.patch<RestaurantTable>(`${API_BASE}/tables/${id}/status`, { status });
    return res.data;
  },
  ...
  // Bookings
  getBookings: async (params?: { status?: string; date?: string; search?: string }) => {
  ...
  updateBookingStatus: async (id: string, status: string, tableId?: string) => {
    const res = await axios.patch<Booking>(`${API_BASE}/bookings/${id}/status`, { status, tableId });
    return res.data;
  },

  // Invoices & Billing
  getInvoices: async (params?: { search?: string; paymentMethod?: string; from?: string; to?: string }) => {
  ...
  createInvoice: async (data: {
    ...
    const res = await axios.post<Invoice>(`${API_BASE}/invoices`, data);
    return res.data;
  },

  // Dashboard
  getDashboardData: async (period?: 'today' | 'week' | 'month' | 'all') => {
    const res = await axios.get<DashboardData>(`${API_BASE}/dashboard`, { params: period ? { period } : undefined });
    return res.data;
  },

  // Restaurant settings
  getSettings: async () => {
    const res = await axios.get<RestaurantSettings>(`${API_BASE}/settings`);
    return res.data;
  },
  updateSettings: async (data: Partial<RestaurantSettings>) => {
    const res = await axios.put<RestaurantSettings>(`${API_BASE}/settings`, data);
    return res.data;
  },
};
